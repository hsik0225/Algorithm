...
public class Baekjoon2775 {
    public static void main(String[] args) {
        ...
        while (n-- > 0) {
            ...
            int[] downFloor = new int[room+1];
            int[] upFloor = new int[room+1];

            for (int i = 1; i <= room; i++) {
                downFloor[i] = i;
            }

            for (int i = 1; i <= floor; i++, downFloor = upFloor) {
                upFloor = new int[room+1];

                for (int j = 1; j <= room; j++) {
                    for (int k = 1; k <= j; k++) {
                        upFloor[j] += downFloor[k];
                    }
                }
            }

            System.out.println(upFloor[room]);
        }
    }
}
